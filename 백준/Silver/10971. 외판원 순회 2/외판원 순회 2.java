...
public class Main {
	...
	private static void dfs(int start, int now, int index, int currentCost) {
		
		if(index == N - 1) { // 모든 노드를 방문했을 경우 N-1 <= 첫 노드를 Main문에서 방문했기 때문에
			// 마지막 노드에서 맨 처음 노드로 가는 비용 포함하기
			if(W[now][start] != 0) {
				currentCost += W[now][start];
		...
		for(int i = 0; i < N; i++) {
			...
			visit[i] = true;
			dfs(start, i, index + 1, currentCost + W[now][i]);
			visit[i] = false;
